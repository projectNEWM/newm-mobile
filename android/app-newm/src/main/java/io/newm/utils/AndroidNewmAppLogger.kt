...
/**
 * An Android-specific implementation of [AppLogger] that logs messages using Android's [Log] class
 * and integrates with Sentry for error tracking.
 */
internal class AndroidNewmAppLogger(
    ...
    /**
     * Logs an error using Android's [Log] and optionally reports the exception to Sentry. Certain
     * exceptions, like [CancellationException] and [IOException], are excluded from reporting.
     *
     * @param tag The tag identifying the source of the log message.
    ...
    override fun error(
        ...
        if (exceptionsNotToReport.none { it.isInstance(exception) }) {
            ...
            Log.w(
                tag,
                "Excluded from Sentry reporting: ${exception::class.simpleName} - $message",
                exception,
            )
        }
    }
    ...
    /**
     * Logs a breadcrumb message to Sentry. Breadcrumbs are used to leave a trail of events or
     * actions that can help diagnose issues.
     *
