...
class NFTLibraryPresenter(
    ...
    override fun present(): NFTLibraryState {
        ...
        LaunchedEffect(Unit) { syncWalletConnectionsUseCase.syncWalletConnectionsFromNetworkToDevice() }
        ...
        var filters: NFTLibraryFilters by rememberRetained {
            mutableStateOf(NFTLibraryFilters(sortType = NFTLibrarySortType.None, showShortTracks = false))
        }
        ...
        val currentTrackId =
            musicPlayer?.let {
                ...
                remember(playbackStatus) {
                    playbackStatus.track?.id.takeIf { playbackStatus.state != PlaybackState.BUFFERING }
                }
            }
        ...
        val downloadStates by
            remember(nftTracks) {
                combine(
                    nftTracks.map { track ->
                        downloadManager.getDownloadState(track.id).map { state -> track.id to state }
                    },
                ) { states ->
                    states.toMap()
                }
                    ...
                    eventSink = { event ->
                        when (event) {
                            is NFTLibraryEvent.OnDownloadTrack -> {
                                downloadManager.download(id = event.track.id, url = event.track.audioUrl)
                            }
                            ...
                            is NFTLibraryEvent.PlaySong -> {
                                val trackIndex = playList.tracks.indexOfFirst { it.id == event.track.id }
