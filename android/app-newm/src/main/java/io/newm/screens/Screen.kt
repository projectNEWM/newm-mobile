...
sealed class Screen(
    ...
    data object Wallets :
        Screen(screenName = AppScreens.WalletsScreen.name, showBottomBar = true, showMiniPlayer = true)

    data class WalletDetail(
        val walletId: String,
        val walletName: String,
    ) : Screen(screenName = AppScreens.WalletDetailScreen.name)

    data object UserAccount :
        Screen(screenName = AppScreens.AccountScreen.name, showBottomBar = true, showMiniPlayer = true)

    data object RecordStore :
        Screen(
            screenName = AppScreens.RecordStoreScreen.name,
            showBottomBar = true,
            showMiniPlayer = true,
        )

    data object NFTLibrary :
        Screen(
    ...
    data object InvestmentPortfolio :
        Screen(
            screenName = AppScreens.InvestmentPortfolioScreen.name,
            showBottomBar = true,
            showMiniPlayer = true,
        )
    ...
    data object Marketplace :
        Screen(
            screenName = AppScreens.MarketplaceScreen.name,
            showBottomBar = true,
            showMiniPlayer = true,
        )
    ...
    data object TermsOfService :
        Screen(screenName = AppScreens.TermsOfServiceScreen.name), WebBrowserScreen {
        @IgnoredOnParcel override val url: String = "https://newm.io/app-tos"
    }

    data object PrivacyPolicy :
        Screen(screenName = AppScreens.PrivacyPolicyScreen.name), WebBrowserScreen {
        @IgnoredOnParcel override val url: String = "https://newm.io/app-privacy"
    }
}
