...
class ProfilePresenter(
    ...
    override fun present(): ProfileUiState {
        ...
        val user by
            remember { userDetailsUseCase.fetchLoggedInUserDetailsFlow() }.collectAsState(null)
        ...
        val showMultiWallets by
            featureFlagService
                .observeFlag(FeatureFlags.ShowMultiWallets)
                .collectAsState(initial = FeatureFlags.ShowMultiWallets.defaultValue)

        val showStudio by
            featureFlagService
                .observeFlag(FeatureFlags.ShowNEWMStudio)
                ...
                eventSink = { event ->
                    when (event) {
                        ...
                        OnDisconnectWallet -> {
                            coroutineScope.launch {
                                eventLogger.logClickEvent(
                                    AppScreens.AccountScreen.DISCONNECT_WALLET_BUTTON,
                                )
                        ...
                        OnShowPrivacyPolicy -> {
                            eventLogger.logClickEvent(
                                AppScreens.AccountScreen.PRIVACY_POLICY_BUTTON,
                            )
