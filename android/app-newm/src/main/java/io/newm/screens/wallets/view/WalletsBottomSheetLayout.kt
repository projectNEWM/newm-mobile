        ...
        sheetContent = {
            ...
            if (type == BottomSheetType.DISCONNECT) {
                ...
                RenameContent(
                    ...
                    state = state,
                    wallet =
                        requireNotNull(selectedWalletConnection) {
...
private fun RenameContent(
    ...
    Column(
        ...
        Row(
            modifier =
                Modifier.width(IntrinsicSize.Min).padding(horizontal = 16.dp, vertical = 8.dp),
        ...
        Text(
            ...
            text =
                stringResource(R.string.wallets_rename_text_label).uppercase(Locale.getDefault()),
                ...
                scope.launch {
                    walletState.eventSink(
                        WalletsEvent.OnRenameWallet(
                            walletId =
                                requireNotNull(wallet.id) { "selectedWalletId should not be null" },
                            newName = newName,
                        ),
                    )
                    state.hide()
...
private fun DisconnectContent(
    ...
    Column(
        modifier =
            Modifier.fillMaxWidth().background(MaterialTheme.colors.background).padding(16.dp),
