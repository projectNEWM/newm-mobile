package io.newm.screens.library

val savedSongModels = listOf(
    SongModel(
        title = "Pink Dream",
        artist = "Jules May",
        imageUrl = "https://i.redd.it/o676ax0heme71.png",
    ),
    SongModel(
        title = "Happy Cooking",
        artist = "Myan Mia",
        imageUrl = "https://static-cse.canva.com/blob/1078769/1600w-fxYFTKLArdY.jpg",
    ),
    SongModel(
        title = "Sunset Vibes",
        artist = "Zac Forland",
        imageUrl = "https://static-cse.canva.com/blob/1078773/1600w-1Nr6gsUndKw.jpg",
    ),
    SongModel(
        title = "Jogging",
        artist = "Disali Merry",
        imageUrl = "https://i.guim.co.uk/img/media/a561b92bd05b3caf63e0a592d2c9d0b25710bad5/0_302_3200_1920/master/3200.jpg?width=1200&height=1200&quality=85&auto=format&fit=crop&s=7c9b5827f44d8134f6433b9a3241fc69",
    ),
    SongModel(
        title = "Go all out",
        artist = "Lopesiada",
        imageUrl = "https://i.redd.it/o676ax0heme71.png",
    ),
    SongModel(
        title = "Relaxing at home",
        artist = "The Arquitect",
        imageUrl = "https://hips.hearstapps.com/hmg-prod/images/soothing-decorating-ideas-dobbies-1648154916.jpg",
    ),
    SongModel(
        title = "Mindless Work",
        artist = "Abstract",
        imageUrl = "https://images.pexels.com/photos/3183150/pexels-photo-3183150.jpeg?cs=srgb&dl=pexels-fauxels-3183150.jpg&fm=jpg",
    ),
    SongModel(
        title = "I don't give af",
        artist = "Mewmouth",
        imageUrl = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQOf0OWS3dkZael9oZqn7Dx-jii9eLjLzP3Jw&usqp=CAU",
    )
)

val libraryArtistListModels = listOf(
    LibraryArtistModel(
        name = "Bruno Mars",
        songCount = 4,
        imageUrl = "https://flxt.tmsimg.com/assets/575770_v9_bg.jpg"
    ),
    LibraryArtistModel(
        name = "Ed Sheeran",
        songCount = 8,
        imageUrl = "https://ca-times.brightspotcdn.com/dims4/default/e28f00c/2147483647/strip/true/crop/3500x2333+0+0/resize/840x560!/quality/80/?url=https%3A%2F%2Fcalifornia-times-brightspot.s3.amazonaws.com%2F17%2Fe4%2F01486f774df688d37a5d12bbcb69%2Fjazz-fest-26819.jpg"
    ),
    LibraryArtistModel(
        name = "Lady Gaga",
        songCount = 12,
        imageUrl = "https://cdn.britannica.com/57/136557-050-0FFC3C93/Lady-Gaga-ceremony-Grammy-Awards-Los-Angeles-Jan-31-2010.jpg"
    ),
    LibraryArtistModel(
        name = "Bad Bunny",
        songCount = 7,
        imageUrl = "https://yt3.googleusercontent.com/7tCfeCWH4arhsTM-4Rz4IxWieQbegzibeXlG-kbytAujdk5dr2K0gBb8NG0Cvk6lB1dPkjyd=s900-c-k-c0x00ffffff-no-rj"
    ),
    LibraryArtistModel(
        name = "Rosalia",
        songCount = 10,
        imageUrl = "https://i.insider.com/5d4d84ff21214c4fe5514038?width=1136&format=jpeg"
    ),
    LibraryArtistModel(
        name = "Rihanna",
        songCount = 15,
        imageUrl = "https://assets.vogue.com/photos/63e67c2653cf7705e1e06f3a/4:3/w_2560%2Cc_limit/GettyImages-1246958447.jpg"
    ),
    LibraryArtistModel(
        name = "Kendrick Lamar",
        songCount = 4,
        imageUrl = "https://www.nme.com/wp-content/uploads/2022/08/Kendrick-Lamar-2000-x-1270.jpg"
    ),
    LibraryArtistModel(
        name = "J. Cole",
        songCount = 8,
        imageUrl = "https://media.pitchfork.com/photos/609d56da35e34915b3b56668/4:3/w_1700,h_1275,c_limit/J-Cole.jpg"
    )
)

val savedAlbumsModels = listOf(
    AlbumModel(
        title = "The Forest Fall",
        genre = "R&B",
        imageUrl = "https://i.pinimg.com/originals/ae/78/41/ae7841d2c1f2fd60a863d97c5ce2446b.jpg",
    ),
    AlbumModel(
        title = "The Rejecteds",
        genre = "Pop",
        imageUrl = "https://www.billboard.com/wp-content/uploads/2022/03/20.-Joy-Division-%E2%80%98Unknown-Pleasures-1979-album-art-billboard-1240.jpg?w=600",
    ),
    AlbumModel(
        title = "Lucia Melin",
        genre = "Pop",
        imageUrl = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTKYEc3YP0R2wv0jk4SMyOoHosddzzSeApTEw&usqp=CAU",
    ),
)
