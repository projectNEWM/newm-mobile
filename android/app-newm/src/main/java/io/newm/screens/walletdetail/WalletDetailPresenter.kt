...
class WalletDetailPresenter(
    ...
    override fun present(): WalletDetailUiState {
        /**
         * Grabbing the wallet connection from the cache, in this case [EmptyWallet] is the init
         * value so we're checking that for the loading state If we don't get a wallet from the
         * cache, we receive null - that's the error state because we should not be in this screen
        ...
        return when (walletConnection) {
            NFTLibraryState.EmptyWallet -> {
                WalletDetailUiState.Loading(eventSink, isSyncing, walletName)
            }

            null -> {
                WalletDetailUiState.Error(eventSink, isSyncing, walletName)
            }
