...
class WalletDetailPresenter(
    ...
    override fun present(): WalletDetailUiState {
         ...
         * If we don't get a wallet from the cache, we receive null - that's the error state
        ...
        val nftTracks: List<NFTTrack> by remember {
            nftTracksUseCase.getAllCollectableTracksFlow()
        }.collectAsRetainedState(initial = emptyList())

        val streamTokens: List<NFTTrack> by remember {
            nftTracksUseCase.getAllStreamTokensFlow()
        }.collectAsRetainedState(initial = emptyList())
            ...
            if (connection != null) {
                recaptchaClientProvider.get()
                    ...
                    .onSuccess { token ->
                        ...
                        logger.error(TAG, "Error getting recaptcha token", it)
                    }
            }
        }
        ...
        return when (walletConnection) {
            NFTLibraryState.EmptyWallet -> WalletDetailUiState.Loading(
                eventSink,
                isSyncing,
                walletName
            )
