        ...
        Presenter.Factory { screen, navigator, _ ->
            when (screen) {
                is CreateAccountScreen ->
                    inject<CreateAccountScreenPresenter> { parametersOf(::launchHomeActivity) }.value
    ...
    private fun buildUiFactory(): Ui.Factory =
        Ui.Factory { screen, _ ->
            when (screen) {
                ...
                is LoginScreen -> ui<LoginScreen.UiState> { state, modifier -> LoginUi(state, modifier) }
...
fun WelcomeToNewm(
    ...
    val newmNavigator =
        rememberNewmNavigator(
            ...
            logger,
            onStartHomeActivity,
            launchBrowser = { url -> context.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(url))) },
