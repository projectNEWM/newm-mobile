...
actual interface OSDependencyProvider {
    ...
    @Provides fun providesHttpClientEngine(): HttpClientEngine = Android.create()
    ...
    ): TokenManager = TokenManagerImpl(context, accountManager)

    @Provides
    fun providesCloudinaryManager(
        context: Context,
        newmCloudinaryAPI: NewmCloudinaryAPI,
        logger: NewmAppLogger,
    ): CloudinaryManager = CloudinaryManagerImpl(context, newmCloudinaryAPI, logger)

    @Provides fun providePlatformName(): String = "Android"
}
