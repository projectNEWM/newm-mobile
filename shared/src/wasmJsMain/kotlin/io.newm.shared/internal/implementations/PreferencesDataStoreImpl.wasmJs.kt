 ...
 *
 * Note: Browser localStorage is synchronous and limited to ~5MB. For larger storage needs, consider
 * migrating to IndexedDB. All operations complete immediately in the browser's single-threaded
 * environment.
 */
class PreferencesDataStoreImpl : PreferencesDataStore {
    ...
    override suspend fun getString(key: String): String? = localStorage.getItem(stringKey(key))
    ...
    override suspend fun getInt(key: String): Int? = localStorage.getItem(intKey(key))?.toIntOrNull()
    ...
    override suspend fun getLong(key: String): Long? = localStorage.getItem(longKey(key))?.toLongOrNull()
    ...
    override suspend fun getBoolean(key: String): Boolean? = localStorage.getItem(boolKey(key))?.toBooleanStrictOrNull()
