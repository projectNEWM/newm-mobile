...
class UserAPI(
    ...
    suspend fun getCurrentUser(): User = authHttpClient.get("/v1/users/me") { contentType(ContentType.Application.Json) }.body()

    suspend fun getUserById(userId: String): User =
        authHttpClient.get("/v1/users/$userId") { contentType(ContentType.Application.Json) }.body()
    ...
    suspend fun deleteCurrentUser() = authHttpClient.delete("/v1/users/me") { contentType(ContentType.Application.Json) }
    ...
    suspend fun updateUserProfile(userProfileUpdateRequest: UserProfileUpdateRequest) {
        try {
            ...
            // TODO:  We need api level errors returned from the api (not http level) so we know what went
            // wrong.  401 here can mean "bad auth token" or "wrong password"
...
@Serializable data class UserCount(
    val count: Int,
)

sealed class UserAPIException(
    message: String,
) : KMMException(message) {
    data class WrongPassword(
        override val message: String,
    ) : UserAPIException(message)

    data class MalformedPassword(
        override val message: String,
    ) : UserAPIException(message)
}

// TODO: this is currently how the server has errors formatted, but we need to fix it.  That code
// should be application level, not http.
@Serializable class APIErrorResponse(
    val code: Int,
