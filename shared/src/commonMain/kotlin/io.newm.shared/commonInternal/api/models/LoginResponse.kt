...
sealed class LoginException(
    ...
    data class HumanVerificationFailed(
        override val message: String,
    ) : LoginException(message)
}

internal fun LoginResponse.isValid(): Boolean = accessToken?.isNotBlank() == true && refreshToken?.isNotBlank() == true
...
sealed class RegisterException(
    ...
    data class TwoFactorAuthenticationFailed(
        override val message: String,
    ) : RegisterException(message)
}
