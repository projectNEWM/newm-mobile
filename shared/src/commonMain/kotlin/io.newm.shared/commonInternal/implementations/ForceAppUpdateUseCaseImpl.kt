...
internal class ForceAppUpdateUseCaseImpl(
    ...
    ): Boolean = shouldForceAppUpdate(currentAppVersion, humanVerificationCode) { it.android.minAppVersion }

    override suspend fun isiOSUpdateRequired(
        currentAppVersion: String,
        humanVerificationCode: String,
    ): Boolean = shouldForceAppUpdate(currentAppVersion, humanVerificationCode) { it.ios.minAppVersion }
