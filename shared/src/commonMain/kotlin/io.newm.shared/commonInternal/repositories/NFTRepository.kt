...
internal class NFTRepository(
    ...
    suspend fun getAllCollectableTracks(): List<NFTTrack> = nftStore.get(Unit).filterNot { it.isStreamToken }

    suspend fun getAllStreamTokens(): List<NFTTrack> = nftStore.get(Unit).filter { it.isStreamToken }

    fun getAllCollectableTracksFlow(): Flow<List<NFTTrack>> = getAll().map { tracks -> tracks.filter { !it.isStreamToken } }

    fun getAllStreamTokensFlow(): Flow<List<NFTTrack>> = getAll().map { tracks -> tracks.filter { it.isStreamToken } }
