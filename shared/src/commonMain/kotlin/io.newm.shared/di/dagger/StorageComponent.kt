...
interface StorageComponent {
    @Provides
    fun providesNftTrackStore(
        service: NFTNetworkService,
        cache: NFTCacheService,
    ): NftTrackStore = NftTrackStore(service, cache)

    @Provides
    fun providesNewmSharedBuildConfig(
        storage: PreferencesDataStore,
        scope: CoroutineScope,
    ): NewmSharedBuildConfig = NewmSharedBuildConfigImpl(storage, scope)
}
