...
class ForceAppUpdateUseCaseTest {
    ...
    private fun setUp() {
        ...
        val fakeConfig =
            MobileConfig(
                version = 1,
                android =
                    MobileClientConfig("1.0.2"), // You might want to adjust these based on the test
    ...
    @Test
    fun `test Android update required when current version is less`() =
        runTest {
            setUp()
            assertTrue(
    ...
    @Test
    fun `test iOS update not required when current version is greater`() =
        runTest {
            setUp()
            assertFalse(
