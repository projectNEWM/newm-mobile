...
class LoginPresenterTest {
    ...
    @Test
    fun `initial state has disabled submit button`() = runTest {
        val presenter = LoginPresenter(navigator, { loginUseCase }, recaptchaManager, logger, analyticsTracker)
    ...
    @Test
    fun `submit button enabled when form is valid`() = runTest {
        val presenter = LoginPresenter(navigator, { loginUseCase }, recaptchaManager, logger, analyticsTracker)
    ...
    @Test
    fun `successful login flow`() = runTest {
        val presenter = LoginPresenter(navigator, { loginUseCase }, recaptchaManager, logger, analyticsTracker)
    ...
    @Test
    fun `recaptcha failure shows error`() = runTest {
        ...
        presenter.test<LoginScreen.UiState> {
            ...
            // Using expectMostRecentItem will give us the latest state after the coroutine finishes.
    ...
    @Test
    fun `login failure sets isLoading to false`() = runTest {
